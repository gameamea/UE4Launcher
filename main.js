...
function saveLocalAssets(cb) {
  if (!localAssetsData || !localAssetsData.length) {
    ...
    fs.writeFile(
      localAssetsPath,
      JSON.stringify(localAssetsData, "", "    "),
      function onwrite() {
        if (cb) {
          cb();
        }
...
function login(cb) {
  ...
  contents.on(
    ...
    function (e, isMainFrame, frameProcessId, frameRoutingId) {
...
function downloadVaultData(cb) {
  ...
  (function loop() {
    ...
    url =
      "https://www.unrealengine.com/marketplace/api/assets/vault?start=" +
      dlIndex +
      "&count=" +
      dlCount;
  ...
  if (offline) {
    ...
    try {
      ...
      if (
        ...
        Date.now() - fs.statSync(vaultPath).mtime.valueOf() <
          1000 * 60 * 60 * 18
      ) {
...
function getEngineVersion(path) {
  ...
  try {
    ...
    data = JSON.parse(
      fs.readFileSync(p.join(path, "Engine", "Build", "Build.version"), "utf8")
    );
...
ipc.on("addAssetToProject", function (e, data) {
  ...
  resId = {
    asset: data.assetData.catalogItemId,
  ...
  if (data.assetData.isLocal) {
    assetAPI.moveToProject(
      data.assetData.path,
      data.projectData.dir,
      function ondone() {
    ...
    assetAPI.addAssetToProject(
      ...
      function onprogress(progress) {
        e.reply(
          "addingAssetProgress",
          JSON.stringify({ id: resId, progress: progress })
        );
      }
    );
  }
