...
class Contextual {
  ...
  constructor(opts) {
    ...
    this.menuControl = contextualCore.CreateEl(
      `<div class='contextualJs contextualMenu'></div>`
    );
    ...
    switch (opts.type) {
      case "seperator":
        this.element = contextualCore.CreateEl(
          `<div class='contextualJs contextualMenuSeperator'><span></span></div>`
        );
        break;
      case "custom":
        this.element = contextualCore.CreateEl(
          `<div class='contextualJs contextualCustomEl'>${opts.markup}</div>`
        );
        break;
      case "submenu":
      case "normal":
        ...
        this.element = contextualCore.CreateEl(`
                    <div class='contextualJs contextualMenuItemOuter'>
                        <div class='contextualJs contextualMenuItem ${
                          opts.enabled == true || opts.enabled == undefined
                            ? ""
                            : "disabled"
                        }'>
                            ${
                              opts.icon != undefined
                                ? `<img src='${opts.icon}' class='contextualJs contextualMenuItemIcon'/>`
                                : `<div class='contextualJs contextualMenuItemIcon ${
                                    opts.cssIcon != undefined
                                      ? opts.cssIcon
                                      : ""
                                  }'></div>`
                            }
                            <span class='contextualJs contextualMenuItemTitle'>${
                              opts.label == undefined ? "No label" : opts.label
                            }</span>
                            <span class='contextualJs contextualMenuItemOverflow ${
                              opts.type === "submenu" ? "" : "hidden"
                            }'>
                                <span class='contextualJs contextualMenuItemOverflowLine'></span>
                                <span class='contextualJs contextualMenuItemOverflowLine'></span>
                                <span class='contextualJs contextualMenuItemOverflowLine'></span>
                            </span>
                            <span class='contextualJs contextualMenuItemTip'>${
      ...
      menu.style.left =
        el.target.offsetLeft + menu.offsetWidth >= window.innerWidth
          ? el.target.offsetLeft -
            menu.offsetWidth +
            el.target.offsetWidth +
            "px"
          : el.target.offsetLeft + "px";

      menu.style.top =
        el.target.offsetTop + menu.offsetHeight >= window.innerHeight
          ? el.target.offsetTop - menu.offsetHeight + "px"
          : el.target.offsetHeight + el.target.offsetTop + "px";
    } else {
      menu.style.left =
        el.clientX + menu.offsetWidth >= window.innerWidth
          ? el.clientX - menu.offsetWidth + "px"
          : el.clientX + "px";

      menu.style.top =
        el.clientY + menu.offsetHeight >= window.innerHeight
          ? el.clientY - menu.offsetHeight + "px"
          : el.clientY + "px";
    }
  },
  CloseMenu: () => {
    let openMenuItem = document.querySelector(
      ".contextualMenu:not(.contextualMenuHidden)"
    );
