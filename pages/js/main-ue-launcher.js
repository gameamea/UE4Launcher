...
function createProjectList() {
  ...
  projects.forEach(function (project) {
    ...
    img.style.backgroundImage =
      'url("' + (project.thumb || defaultThumb) + '")';
...
function launchEngine(engine, project) {
  ...
  if (
    !lastProjectLaunchedTime ||
    curTime - lastProjectLaunchedTime > 5000 ||
    ...
    console.log(
      "Launching " + engine.execPath + (args ? " " + args.join(" ") : "")
    );
...
function hasDownloaded(assetData, version) {
  ...
  if (assetsData[id]) {
    ...
    for (i = keys.length - 1; i >= 0; --i) {
      if (
        assetsData[id][keys[i]].engineVersion === version &&
        assetsData[id][keys[i]].downloaded
      ) {
        return true;
      }
    }
...
function createAddProjectMenuItems(
  assetData,
  assetContainerEl,
  assetImageEl,
  isLocal
  ...
  var items = [
    new ContextualItem({
      type: "custom",
      markup: "<strong>Add to Project</strong>",
    }),
  ];
  ...
  if (!isLocal) {
    ...
    items.push(
      new ContextualItem({
        type: "custom",
        markup: "<strong>Download to Cache</strong>",
      })
    );
    ...
    configData.engines.forEach(function (engine) {
      items.push(
        new ContextualItem({
          ...
          label:
            engine.version +
            (hasDownloaded(assetData, engine.version) ? " \u2714" : ""),
          onClick: function () {
            addAssetToProject(
              assetData,
              { downloadOnly: true, version: engine.version },
              assetContainerEl,
              assetImageEl
            );
          },
    ...
    items.push(
      new ContextualItem({
        ...
        onClick: function () {
          simpleAsyncPrompt(
            "Enter Engine Version to Download",
            function (version) {
              if (version) {
                if (version[0] === ".") {
                  version = "4" + version;
                ...
                addAssetToProject(
                  assetData,
                  { downloadOnly: true, version: version },
                  assetContainerEl,
                  assetImageEl
                );
              }
