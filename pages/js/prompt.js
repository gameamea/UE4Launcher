...
(function (name, context, definition) {
  ...
  PromptBoxes.prototype = {
    ...
    clear: function () {
      ...
      var elements = [];
      var classes = [
        that._prefixes.backdrop,
        that._prefixes.alert,
        that._prefixes.confirm,
        that._prefixes.prompt,
      ];
    ...
    _addToast: function (el$, className, opts) {
      ...
      if (that._toastQueue.length >= opts.max) {
        that._toastQueue.splice(
          opts.max - 1,
          that._toastQueue.length - (opts.max - 1)
        );
      }
