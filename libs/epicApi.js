...
var login = (function () {
  ...
  function webLogin(user, pass, ondone, onerror, onprogress) {
    ...
    request.post(opts, function (err, res, body) {
      ...
      } else if (!err && (res.statusCode == 302 || res.statusCode == 200)) {
        // success
        request._updateFakeJar(res.headers["set-cookie"]);
  ...
  function webAuthorize(ondone, onerror, onprogress) {
    var opts = {
      ...
      headers: {
        ...
        //Host: "accounts.unrealengine.com",
        "User-Agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) EpicGamesLauncher/10.18.1-13954725+++Portal+Release-Live UnrealEngine/4.23.0-13954725+++Portal+Release-Live Chrome/59.0.3071.15 Safari/537.36",
        Referer: "https://www.epicgames.com/id/login/welcome",
        "X-Epic-Event-Action": "login",
        "X-Epic-Event-Category": "login",
        "X-Epic-Strategy-Flags":
          "guardianEmailVerifyEnabled=false;guardianEmbeddedDocusignEnabled=true;guardianKwsFlowEnabled=false;minorPreRegisterEnabled=false",
    ...
    var opts = {
      ...
      headers: {
        ...
        "X-Epic-Correlation-ID":
          ...
          "-E5E6229A474CBD0CD937F2A211029A98-BA82B58E4F388B14AD8D2694E9FB47B2",
        "User-Agent":
          "UELauncher/10.18.1-13954725+++Portal+Release-Live Windows/10.0.10240.1.256.64bit",
  ...
  webAuthorize.withUsernameAndPass = function login(
    user,
    pass,
    ondone,
    onerror,
  ...
  fs.readFile(path, "utf8", function onread(err, data) {
    ...
    if (!err && data) {
      try {
        ...
        if (
          Date.now() - fs.statSync(path).mtime.valueOf() <
          1000 * 60 * 60 * 24 * 7
        ) {
...
function downloadAssetInfo(catalogItemId, cb) {
  var opts = {
    ...
    headers: {
      ...
      Origin: "allar_ue4_marketplace_commandline",
      "User-Agent":
        "game=UELauncher, engine=UE4, build=allar_ue4_marketplace_commandline",
    },
    qs: {
      id: catalogItemId,
      country: "US",
  ...
  fs.readFile(path, "utf8", function onread(err, data) {
    ...
    if (!err && data) {
      try {
        ...
        if (
          skipCache /*(json.expires && (new Date(json.expires)).valueOf() < Date.now()) ||*/ ||
          Date.now() - fs.statSync(path).mtime.valueOf() >
            1000 * 60 * 60 * 24 * 7
        ) {
          0;
    ...
    if (err || !json) {
      authenticateIfNecessary(
        function () {
          downloadItemBuildInfo(
            catalogItemId,
            appId,
            function (err, itemBuildInfo) {
...
function selectDistributionHost(itemBuildInfo, hostNum) {
  ...
  if (hostNum < 0) {
    ...
    itemBuildInfo.items.MANIFEST.additionalDistributions &&
    hostNum < itemBuildInfo.items.MANIFEST.additionalDistributions.length
  ) {
    distributionName =
      itemBuildInfo.items.MANIFEST.additionalDistributions[hostNum];
  }
...
function downloadItemManifest(itemBuildInfo, hostNum, useAuth, cb) {
  ...
  var opts = {
    uri:
      distributionName +
      itemBuildInfo.items.MANIFEST.path +
      "?" +
      itemBuildInfo.items.MANIFEST.signature,
    headers: {
      Origin: "allar_ue4_marketplace_commandline",
      "User-Agent":
        "game=UELauncher, engine=UE4, build=allar_ue4_marketplace_commandline",
...
function chunkHashToReverseHexEncoding(chunkHash) {
  ...
  for (i = 0; i < 8; ++i) {
    outHex =
      byteToHex(parseInt(chunkHash.substring(i * 3, i * 3 + 3))) + outHex;
  }
  return outHex;
}
...
function buildItemChunkList(itemBuildInfo, manifest) {
  ...
  var filename;
  var chunkBaseURL =
    itemBuildInfo.items.CHUNKS.path.substr(
      0,
      itemBuildInfo.items.CHUNKS.path.indexOf("/CloudDir")
    ) + "/CloudDir/ChunksV3/";
...
function isHashCorrect(data, expectedHash) {
  return (
    crypto.createHash("sha1").update(data).digest("hex").toUpperCase() ===
    expectedHash
  );
}
      ...
      mkdirs(p.dirname(fileName), extractedBasePath, function extractChunk() {
        fs.open(fileName, "w", function onopen(err, outFile) {
          ...
          (function loopFileChunkParts(chunkCount) {
            ...
            var offset = parseInt(
              "0x" + chunkHashToReverseHexEncoding(chunkPart.Offset)
            );
            var size = parseInt(
              "0x" + chunkHashToReverseHexEncoding(chunkPart.Size)
            );
            var hash = chunkHashToReverseHexEncoding(
              manifest.ChunkHashList[guid]
            );
      ...
      login(
        ...
        onprogress ||
          function progress(amount, message, total) {
            console.log(
              message + " " + Math.round((amount / total) * 100) + "%"
            );
          }
      );
    });
...
function addAssetToProject(
  ...
  loadAssetCache(function () {
    ...
    getAssetInfo(id, function (err, assetInfo, skipCache) {
      ...
      getItemBuildInfo(
        id,
        versions[projectVersion].appId,
        skipCache,
        function (err, itemBuildInfo, skipCache) {
